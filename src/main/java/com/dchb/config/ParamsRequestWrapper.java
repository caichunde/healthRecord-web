    ...
    ParamsRequestWrapper(HttpServletRequest request, boolean b) {
        ...
        if (b) {
            try {
                ...
                allParamStr = getBody(super.getInputStream());
            } catch (Exception e) {
            ...
            if (super.getMethod().equals("GET")) {
                ...
                for (Iterator itr = keySet.iterator(); itr.hasNext(); ) {
                    ...
                    for (int k = 0; k < value.length; k++) {
                        ...
                        allParamStr = content.replaceAll(" ", "+");
                        allParamStr = allParamStr.replaceAll("}", "=}");
                    }
                }
            }
            // 获取加密参数字符串
            int closeLen = allParamStr.length() - 1;
            int openLen = "{\"request\":".length();
            String substring = StringUtils.substring(allParamStr, openLen, closeLen);
            // 解密
