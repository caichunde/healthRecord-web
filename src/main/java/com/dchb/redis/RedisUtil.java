...
public class RedisUtil {
	...
	/**
	 * 有序集合 获取所有元素并显示分数,按分数升序
	 * 
	 * @param key		键名
	 * @return Set<TypedTuple<Object>>
	 */
	public Set<TypedTuple<Object>> rangeWithScores(String key) {
		ZSetOperations<String, Object> zset = redisTemplate.opsForZSet();
	 ...
	 * @return
	 */
	public void zIncr(String key, Object value, double scoure) {
		ZSetOperations<String, Object> zset = redisTemplate.opsForZSet();
		zset.incrementScore(key, value, scoure);
	}
	
	/**
	 * <p>Title: reverseRange</p>  
	 * <p>Description: 获取按分数倒序前几个值</p>  
	 * @param key 
	 * @param count 获取值个数
	 * @return
	 */
	public Set<Object> reverseRange(String key,int count) {
		ZSetOperations<String, Object> zset = redisTemplate.opsForZSet();
		return zset.reverseRange(key, 0, count-1);
		
	}
	
	/**
	 * <p>Title: rank</p>  
	 * <p>Description: 获取zset成员的下标位置，如果值不存在返回null</p>  
	 * @param key
	 * @param value
	 * @return
	 */
	...
	// zset.rangeWithScores(key, 0, -1); //获取所有元素并显示分数,按分数升序,分数相同插入在前排列再前
	
	// zset.zCard(key); // 获取集合长度
	
	// zset.reverseRangeByScore(key, min, max); // 获取集合中分数在区间中的值
